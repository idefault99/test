...
## Шаг 2.1 — Скачивание Ubuntu 22.04 ISO
...
## Шаг 2.2 — Создание preseed/autoinstall для автоматической установки Ubuntu
...
## Шаг 2.3 — Создание VM на GPU-хостах
...
echo "Диск данных: $DATA_DISK"

# Создаём диск для ОС VM (на быстром 3.5TB диске)
...
<domain type='kvm'>
  ...
  <devices>
    ...
    <!-- Диск данных (физический диск напрямую) -->
...
## Шаг 2.4 — Создание VM на storage-хостах
...
virsh define "/tmp/${VM_NAME}.xml"
virsh autostart "${VM_NAME}"
echo "✓ VM ${VM_NAME} создана"
```

## Шаг 2.5 — Запуск VM и установка Ubuntu
...
## Шаг 2.6 — Настройка watchdog-сервиса на хостах
...
## Шаг 2.7 — Проверка доступности VM по SSH
...
## Шаг 3.6 — Подготовка диска данных (vdb)
...
lsblk "$DISK" 2>/dev/null || { echo "ОШИБКА: диск $DISK не найден"; exit 1; }

SIZE_GB=$(lsblk -bnd -o SIZE "$DISK" | awk '{printf "%.0f", $1/1024/1024/1024}')
echo "Диск: $DISK, размер: ${SIZE_GB} GB"

# Проверяем есть ли уже данные
if lsblk -no FSTYPE "$DISK" | grep -qE '\S'; then
  echo "На диске есть ФС. Для форматирования: FORCE=1 $0"
  [ "${FORCE:-0}" = "1" ] || exit 0
  wipefs -a "$DISK"
fi

apt-get install -y gdisk parted xfsprogs >/dev/null

sgdisk --zap-all "$DISK"
sgdisk -n 1:0:0 -t 1:8300 -c 1:"longhorn-data" "$DISK"
partprobe "$DISK"; sleep 2

PART="${DISK}1"
mkfs.xfs -f -L longhorn-data -i size=512 "$PART"
...
systemctl daemon-reload
mount -a
mountpoint -q "$MOUNT" && echo "✓ Диск примонтирован: $MOUNT"
df -hT "$MOUNT"
```
